...
public class YouTube implements MediaProvider {
    ...
    private Long parseLength(String text) {
        ...
        if (m.matches()) {
            ...
            long duration = 0;
            if (hr != null) duration += Long.parseLong(hr) * 60 * 60;
            if (min != null) duration += Long.parseLong(min) * 60;
            if (sec != null) duration += Long.parseLong(sec);
