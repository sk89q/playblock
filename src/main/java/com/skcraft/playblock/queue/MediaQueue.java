 ...
 * "advancing" to the next media file after each media file has elapsed its expected
 * length. Media is "advanced" by firing the callbacks on added {@link QueueListener}s.
 */
public class MediaQueue {
    ...
    private synchronized void advanceTo(Media media) {
        ...
        if (media != null) {
            ...
            queueManager.getQueueTimer().schedule(
                    timerTask = new MediaAdvancer(), media.getLength());
        }
    ...
    /**
     * Advance to the next media in the queue, or stop playing if there's nothing
     * playing.
     */
     ...
     * 
     * <p>The passed media must have its length known.</p>
     * 
     * @param media the media
     */
    ...
    /**
     * Mark this queue for release, indicating that it is no longer to be used. It is
     * important that this method is called in order to release resources.
     * 
     * <p>This will not fire any events on added {@link QueueListener}s.</p>
     */
    ...
    /**
     * Used to advance the media after the duration of the currently "playing" media
     * has elapsed.
     */
