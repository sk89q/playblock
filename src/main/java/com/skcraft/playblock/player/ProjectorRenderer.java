...
public class ProjectorRenderer extends TileEntitySpecialRenderer {
            ...
            if (renderer != null) {
                ...
                if (status == RendererState.INITIALIZING) {
                    ...
                    drawTextBox(0, 0, width, height, true, 0xffff0000,
                            "Couldn't play video;", "maybe bad URL?");
    ...
    private void drawPlayBlockStatus(float width, float height) {
        ...
        GL11.glPushMatrix();
        {
            ...
            bindTextureByName("/playblock/screen_logo.png");
    ...
    private boolean drawLogo(double t, float width, float height) {
        ...
        float alpha = (float) (
                MathUtils.easeInQuad(t, 0, 1, LOGO_DURATION * 0.4) - // Entrance
        ...
        GL11.glPushMatrix();
        {
            ...
            if (width >= height) {
                DrawUtils.drawCenteredTexture(x, y,
                        width * LOGO_SCALE,
                        width * LOGO_SCALE * (LOGO_HEIGHT / (float) LOGO_WIDTH));
            } else {
                DrawUtils.drawCenteredTexture(x, y,
                        height * LOGO_SCALE * (LOGO_WIDTH / (float) LOGO_HEIGHT),
                        height * LOGO_SCALE);
            }
    ...
    private void drawSpinner(double t, float width, float height) {
        ...
        GL11.glPushMatrix();
        {
            ...
            float angle = (int) ((t / 7 /
                    (360 / SPINNER_ANGLE)) % (360 / SPINNER_ANGLE)) * -SPINNER_ANGLE;

            GL11.glTranslatef(x, y, -0.01f);
            GL11.glRotatef(angle, 0, 0, -0.01f);
            GL11.glColor4f(1, 1, 1, 1);
            
            bindTextureByName("/playblock/spinner.png");
            if (width >= height) {
                DrawUtils.drawCenteredTexture(0, 0,
                        width * SPINNER_SCALE, width * SPINNER_SCALE);
            } else {
                DrawUtils.drawCenteredTexture(0, 0,
                        height * SPINNER_SCALE, height * SPINNER_SCALE);
            }
     ...
     */
    private void drawTextBox(float x0, float y0, float x1, float y1, 
            boolean centered, int bgColor, String... lines) {
        ...
        float totalHeight = lines.length * 
                (getFontRenderer().FONT_HEIGHT + LINE_SPACING);
        ...
        GL11.glPushMatrix();
        {
            ...
            for (int i = 0; i < lines.length; i++) {
                ...
                DrawUtils.drawRect(0, a, 
                        textWidths[i], textHeight + a, bgColor);
            }
        }
        GL11.glPopMatrix();
        ...
        GL11.glPushMatrix();
        {
            ...
            for (int i = 0; i < lines.length; i++) {
                getFontRenderer().drawString(lines[i], 0, 
                        i * (textHeight + LINE_SPACING),
                        0xffffffff, false);
            }
        }
        GL11.glPopMatrix();
