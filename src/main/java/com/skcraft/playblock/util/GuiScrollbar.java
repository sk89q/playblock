...
public class GuiScrollbar extends Gui {
    ...
    public GuiScrollbar(Minecraft mc, int xPos, int yPos, int scrollHeight, int textureX, 
            int textureY, int textureWidth, int textureHeight, String texturePath) {
        this.mc = mc;
        this.xPos = xPos;
        this.yPos = yPos;
    ...
    public void drawScrollbar(int mouseX, int mouseY) {
        ...
        if (!wasClicking && mouseDown && mouseX >= xPos
                && mouseX < scrollRight && mouseY >= yPos
                && mouseY < scrollBottom) {
            isScrolling = enabled;
        }
        ...
        if (isScrolling) {
            currentScroll = (mouseY - yPos - (height / 2.0f))
                    / ((float) (scrollBottom - yPos) - height);
        ...
        drawTexturedModalRect(xPos, yPos + (int) ((scrollBottom - yPos - height) * currentScroll),
