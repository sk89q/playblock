...
public class PlayBlockSetup extends JFrame implements ProgressListener {
    ...
    public PlayBlockSetup() {
        ...
        try {
            InputStream in = PlayBlockSetup.class
                    .getResourceAsStream("/playblock/setup/icon.png");
            if (in != null) {
    ...
    private void addComponents() {
        ...
        try {
            InputStream is = PlayBlockSetup.class
                    .getResourceAsStream("/playblock/setup/background.png");
        ...
        selectionPanel.add(new JLabel(
                "Welcome to the PlayBlock installer! Please select a choice:",
                SwingConstants.LEFT));
        ...
        button.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent event) {
                if (SetupUtils.yesNo(parent, 
                        "Are you sure that you wish to delete all the support files?", 
                        "Confirmation")) {
    ...
    private void tryCancelling() {
        if (currentTask != null) {
            if (SetupUtils.yesNo(this, "Are you sure that you want to cancel?", 
                    "Confirmation")) {
                cancel();
                dispose();
            }
    ...
    public void complete() {
        if (currentTask instanceof Install) {
            SetupUtils.showMessageDialog(this, "Installation completed successfully. " +
            		"If you still does not work, try installing in-game.",
                    "Complete", JOptionPane.INFORMATION_MESSAGE);
        } else {
            SetupUtils.showMessageDialog(this, "Completed successfully! This setup tool will now close.",
                    "Complete", JOptionPane.INFORMATION_MESSAGE);
        }
    ...
    public void error(Throwable exception) {
        if (exception instanceof TaskException) {
            SetupUtils.showMessageDialog(this, exception.getMessage(), exception.getCause(),
                    "An error has occurred", JOptionPane.ERROR_MESSAGE);
        } else {
            SetupUtils.showMessageDialog(this, exception.getMessage(), exception,
                    "An error has occurred", JOptionPane.ERROR_MESSAGE);
        }
