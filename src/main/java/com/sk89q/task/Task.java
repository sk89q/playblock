...
/**
 * A task that can be performed in another thread, similar to {@link SwingWorker}.
 */
public abstract class Task implements Runnable {
    ...
    /**
     * Attempts to cancel the task by throwing an {@link InterruptedException} in the
     * thread.
     * 
     * <p>This should not be called if this task is not running in a
     * different thread. In addition, this method is only usable if the task was
     * started with {@link #start()} and not with an external executor.</p>
     * 
     * <p>It is still possible for the task to complete or error after the task has
     * been cancelled.</p>
     */
    public void cancel() {
        if (thread != null) {
