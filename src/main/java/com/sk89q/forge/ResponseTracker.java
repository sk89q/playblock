...
public class ResponseTracker {
    ...
    private final Map<Short, SettableFuture<?>> futures = 
            new HashMap<Short, SettableFuture<?>>();
    ...
    /**
     * Called after receiving a response, on the original sending side, in order to
     * fire the callbacks.
     * 
     * @param trackable the trackable object
     */
    public synchronized <T> void fireFuture(RequestResponse<T> trackable) {
        SettableFuture<T> future = 
                (SettableFuture<T>) futures.remove(trackable.getCallId());
    ...
    /**
     * Called on the receiving side that will issue the response, in order to copy the
     * call ID from the request to the response.
     * 
